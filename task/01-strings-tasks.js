...
function extractNameFromTemplate(value) {
    return value.substr(7, value.length).slice(0, -1);
}
...
function getRectangleString(width, height) {
    ...
    const line2 = `│${spaces}│\n`;
    const lineAmount = line2.repeat(height - 2);
    return `${line1}${lineAmount}${lineN}`;
}
...
function encodeToRot13(str) {
    ...
    const index = (symbol) => input.indexOf(symbol);
    return str.split('').map((s) => ((index(s) !== -1) ? output[index(s)] : s)).join('');
}
